...
func TestGetBearerToken(t *testing.T) {
	h := http.Header{}
	h.Set("Authorization", "Bearer hui-na-blude")
	h.Add("Content-Type", "application/json")
	token, err := GetBearerToken(h)
	if err != nil {
		t.Errorf("Get bearer token failed, error: %v", err)
	}
	if token == "" {
		t.Error("Token should not be empty")
	}
	if token != "hui-na-blude" {
		t.Errorf("Wrong token")
	}

	h1 := http.Header{}
	_, err = GetBearerToken(h1)
	if err == nil {
...
func TestMakeRefreshToken(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Make refresh token failed")
	}
}
